...
class pFedMe(Server):
    def __init__(self, device, dataset, algorithm, model, batch_size, learning_rate, beta, lamda, num_glob_iters,
                 ...
                 poison_label, attack_method, per_epoch, defense, central_dataset, dataset_test, tau, local_bs, plr_class,
                 server_lr, wrong_mal, right_ben, turn, noise, K, personal_learning_rate):
        ...
        for i in range(total_users):
            ...
            user = UserpFedMe(device, id, train, test, model, dataset, batch_size, learning_rate, beta, lamda, local_epochs,
    ...
    def train(self, trigger, per_epoch, attack_start, pattern, oneshot, clip_rate, defense):
        ...
        for glob_iter in range(0, self.num_glob_iters + 1):
            ...
            self.selected_users = self.select_users_fixed(glob_iter, self.num_users)
            ...
            for user in self.selected_users:
                ...
                if user.id in self.malclient:
                    ...
                    if glob_iter >= attack_start and oneshot == 0:
                        ...
                        csv_record.test_local_result.append(
                            [user.id, glob_iter, 'True', accuracy, acc_count, sumcount])
                    ...
                    user.train(self.local_epochs)  # * user.train_samples  #训练本地模型
            ...
            if self.defense == "none":
                ...
                self.Trimmed_Mean()
            elif self.defense == 'geminiguard':
                self.Geminiguard_Aggregate(glob_iter=glob_iter)
            elif self.defense == 'fltrust':
                self.Fltrust_Aggregate()
            elif self.defense == 'flare':
