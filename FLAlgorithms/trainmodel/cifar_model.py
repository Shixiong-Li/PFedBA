...
class ResNet(SimpleNet):
    ...
    def get_feature_list(self, x):
        ...
        feature_list = []
        out = F.relu(self.bn1(self.conv1(x)))
        feature_list.append(out.clone().detach())  # First layer feature
        out = self.layer1(out)
        feature_list.append(out.clone().detach())  # Second layer feature
        out = self.layer2(out)
        feature_list.append(out.clone().detach())  # Third layer feature
        out = self.layer3(out)
        feature_list.append(out.clone().detach())  # Fourth layer feature
        out = self.layer4(out)
        feature_list.append(out.clone().detach())  # Final feature
        out = F.avg_pool2d(out, 4)
        feature_list.append(out.clone().detach())  # Pooled feature
        return feature_list  # Return features from multiple layers
